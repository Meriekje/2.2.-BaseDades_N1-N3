...
@NoArgsConstructor
@AllArgsConstructor
@ToString(exclude = "owner") // Evita problemes de recursió amb Lombok
public class Pet {
    @Id
    ...
    // Mètode per simular el pas del temps (les mascotes es posen més tristes amb el temps)
    public void simulateTimePass(long hoursElapsed) {
        if (hoursElapsed > 0) {
            this.hungerLevel = Math.min(100, this.hungerLevel + (int)(hoursElapsed * 5));
            this.energyLevel = Math.max(0, this.energyLevel - (int)(hoursElapsed * 3));
            this.happinessLevel = Math.max(0, this.happinessLevel - (int)(hoursElapsed * 2));
        }
    }
}
